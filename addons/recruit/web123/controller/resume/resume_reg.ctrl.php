...
if($op=="1"){
    ...
    if($resume){
        ...
        $r = pdo_update("resume",$data,array("uid"=>$_SESSION['uid']));
    }else{
        $data['uid'] = $_SESSION['uid'];
        $data['addtime'] = time();
        $r = pdo_insert("resume",$data);
    }

    if($r){
        call_back(1,web_url("resume/resume_reg/2"));
    }
    ...
    $r = pdo_update("resume",$data,array("uid"=>$_SESSION['uid']));
    if($r){
        call_back(1,web_url("resume/resume_reg/3"));
    }
    ...
    $r = pdo_update("resume",$data,array("uid"=>$_SESSION['uid']));
    if($r){
        call_back(1,web_url("resume/resume_reg/4"));
    }
    ...
    $r = pdo_update("resume",$data,array("uid"=>$_SESSION['uid']));
    if($r){
        call_back(1,web_url("person/index"));
    }
}elseif ($op=="upload"){
