...
if($op=="index"){
    ...
    include wl_template('member/company_reg2');
}elseif($op=="base_save"){
...
//判断是否是通过HTTP POST上传的
        ...
        $upload_path = WL_WEB."resource/file/"; //上传文件的存放路径
        ...
        if(move_uploaded_file($file['tmp_name'],$upload_path_name)){
            ...
            $data['createtime'] = time();
            if($_GPC['kind']==1){
                $data['license'] = WL_URL_WRES."file/".$name;
            }elseif($_GPC['kind']==2){
                $data['idcard1'] = WL_URL_WRES."file/".$name;
            }elseif($_GPC['kind']==3){
                $data['idcard2'] = WL_URL_WRES."file/".$name;
            }
            pdo_insert("members_temp",$data);
            call_back(1,"上传成功") ;
        }else{
            call_back(2,"上传失败") ;
        }
    ...
    $uid = $_SESSION['uid'];
    $company_profile = pdo_fetch("select * from ".tablename('company_profile')." where uid=".$uid);
    $temp = pdo_fetch("select * from ".tablename('members_temp')." where uid=".$uid);
    if($_GPC['companyname']){
    ...
    if($temp['license']){
        ...
        if($company_profile){
            $r = pdo_update("company_profile",$data,array('uid'=>$uid));
        }else{
            $data['uid'] = $uid;
            $data['createtime']=time();
            $r = pdo_insert("company_profile",$data);
        }
    }elseif ($temp['idcard1']){
    ...
    if($r){
        pdo_delete("members_temp",array("uid"=>$uid));
        call_back(1,"上传成功");
    }
    ...
    $company = pdo_fetch("select id from ".tablename('company_profile')." where uid=".$_SESSION['uid']);
    if($company){
        $r = pdo_update("company_profile",$data,array('uid'=>$_SESSION['uid']));
    }else{
        $data['uid'] = $_SESSION['uid'];
        $data['createtime']=time();
        $r = pdo_insert("company_profile",$data);
    }
    call_back(1,web_url("member/company_register/step3"));
}elseif ($op=="step3"){
    include wl_template("member/company_reg3");
    ...
    $r = pdo_update("company_profile",$data,array('uid'=>$_SESSION['uid']));
    if($r){
        call_back(1,web_url("company/index"));
    }
}elseif ($op=="upload"){
    $file = $_FILES['file'];//得到传输的数据
    $name = $file['name'];
    ...
    $upload_path = WL_WEB."resource/file/".date("Y-m-d"); //上传文件的存放路径
    mkdirs($upload_path);
    ...
    //开始移动文件到相应的文件夹
    if(move_uploaded_file($file['tmp_name'],$upload_path_name)){
        call_back(1,WL_URL_WEB."resource/file/".date("Y-m-d")."/".$name);
    }
}
