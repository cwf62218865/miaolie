...
function tpl_form_field_image($name, $value = '', $default = '', $options = array()) {
	...
	$options['fileSizeLimit'] = intval($GLOBALS['_W']['setting']['upload']['images']['limit']) * 1024;
	$s = '';
	...
	$s .= '
		...
		<div class="input-group ' . $options['class_extra'] . '" style="margin-top:.5em;">
			<img src="' . $val . '" onerror="this.src=\'' . $default . '\'; this.title=\'图片未找到.\'" class="img-responsive img-thumbnail" ' . ($options['extras']['images'] ? $options['extras']['images'] : '') . ' width="150" />
...
function tpl_form_field_wechat_image($name, $value = '', $default = '', $options = array()) {
	...
	$options['direct'] = true;
	$options['multiple'] = false;
	$options['type'] = empty($options['type']) ? 'images' : $options['type'];
	$s = '';
...
function tpl_ueditor($id, $value = '', $options = array()) {
	...
	<script type=\"text/javascript\">
		require(['util'], function(util){
			...
			image_limit : " . (intval($GLOBALS['_W']['setting']['upload']['images']['limit']) * 1024) . ",
