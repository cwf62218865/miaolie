...
class CoreModule extends WeModule {
	public $modules = array('basic', 'news', 'images', 'music', 'voice', 'video', 'wxcard', 'keyword', 'module');
		...
		switch($_GPC['a']) {
			case 'mass':
				...
				if(!empty($isexists['media_id']) && !empty($isexists['msgtype'])) {
					...
					switch($isexists['msgtype']) {
						case 'news':
							...
							break;
						case 'images':
							$replies['images'][0]['img_url'] = tomedia($wechat_attachment['attachment'], true);
							$replies['images'][0]['mediaid'] = $isexists['media_id'];
							break;
						case 'voice':
				...
				if(!empty($isexists)) {
					...
					$module = $module == 'images' ? 'images' : $module;
