...
load()->model('material');
load()->model('mc');
if (!in_array($do, array('upload', 'fetch', 'browser', 'delete', 'images' ,'module' ,'video', 'voice', 'news', 'keyword',
...
$type  =  $_GPC['upload_type'];$type = in_array($type, array('images','audio','video')) ? $type : 'images';
$option = array();
...
if ($do == 'fetch' || $do == 'upload') {
	...
	pdo_insert('core_attachment', array(
		...
		'filename' => $originname,
		'attachment' => $pathname,
		'type' => $type == 'images' ? 1 : ($type == 'audio'||$type == 'voice' ? 2 : 3),
...
if ($do == 'images') {
	...
	if ($islocal) { 		$page = $_GPC['page'];
		...
		$material_news_list = material_list('images', MATERIAL_WEXIN, array('page_index' => $page_index, 'page_size' => $page_size));
		$material_list = $material_news_list['material_list'];
		$pager = $material_news_list['page'];
...
if ($do == 'tolocal' || $do == 'towechat') {
	if (!in_array($type, array('news', 'images', 'video', 'voice'))) {
		iajax(1, '转换类型不正确');
		return;
	}
}
...
if ($do == 'networktolocal') {
	$type = $_GPC['type'];
	if (!in_array($type,array('images','video'))) {
		$type = 'images';
	}
