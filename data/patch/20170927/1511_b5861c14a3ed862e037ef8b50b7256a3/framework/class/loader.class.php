...
class Loader {
	...
	function admin($name) {
		global $_W;
		if (isset($this->cache['admin'][$name])) {
			return true;
		}
		$file = IA_ROOT . '/admin/common/' . $name . '.func.php';
		if (file_exists($file)) {
			include $file;
			$this->cache['admin'][$name] = true;
			return true;
		} else {
			trigger_error('Invalid Web Helper /admin/common/' . $name . '.func.php', E_USER_ERROR);
			return false;
		}
	}
