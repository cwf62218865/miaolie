...
function file_upload($file, $type = 'images', $name = '') {
	$harmtype = array('asp', 'php', 'jsp', 'js', 'css', 'php3', 'php4', 'php5', 'ashx', 'aspx', 'exe', 'cgi');
	if (empty($file)) {
		return error(-1, '没有上传内容');
	}
	if (!in_array($type, array('images', 'thumb', 'voice', 'video', 'audio'))) {
	...
	switch ($type) {
		...
		case 'thumb' :
			...
			$limit = $setting['upload']['images']['limit'];
			break;
		case 'voice' :
		case 'audio' :
...
function file_wechat_upload($file, $type = 'images', $name = '') {
	$harmtype = array('asp', 'php', 'jsp', 'js', 'css', 'php3', 'php4', 'php5', 'ashx', 'aspx', 'exe', 'cgi');
	if (empty($file)) {
		return error(-1, '没有上传内容');
	}
...
function file_image_thumb($srcfile, $desfile = '', $width = 0) {
	...
	if (intval($width) == 0) {
		load()->model('setting');
		$width = intval($_W['setting']['upload']['images']['width']);
	}
	if (intval($width) < 0) {
...
function file_remote_attach_fetch($url, $limit = 0, $path = '') {
	...
	if (! $limit) {
		if ($type == 'images'){
			$limit = $_W['setting']['upload']['images']['limit'] * 1024;
		}else{
