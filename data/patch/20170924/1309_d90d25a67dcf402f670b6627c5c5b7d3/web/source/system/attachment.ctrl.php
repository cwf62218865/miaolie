...
if ($do == 'global') {
	...
	if (checksubmit('submit')) {
		...
		$upload = $_GPC['upload'];
		if (!empty($upload['images']['thumb'])) {
			$upload['images']['thumb'] = 1;
		} else {
			$upload['images']['thumb'] = 0;
		}
		$upload['images']['width'] = intval(trim($upload['images']['width']));
		if (!empty($upload['images']['thumb']) && empty($upload['images']['width'])) {
			itoast('请设置图片缩略宽度.', '', '');
		}
		$upload['images']['limit'] = max(0, min(intval(trim($upload['images']['limit'])), $post_max_size));
		if (empty($upload['images']['limit'])) {
			itoast('请设置图片上传支持的文件大小, 单位 KB.', '', '');
		}
		if (empty($upload['images']['extentions'])) {
			itoast('请添加支持的图片附件后缀类型', '', '');
		}
		if (!empty($upload['images']['extentions'])) {
			$upload['images']['extentions'] = explode("\n", $upload['images']['extentions']);
			foreach ($upload['images']['extentions'] as $key => &$row) {
				$row = trim($row);
				if (in_array($row, $harmtype)) {
					unset($upload['images']['extentions'][$key]);
					continue;
				}
			}
		}
		if (!is_array($upload['images']['extentions']) || count($upload['images']['extentions']) < 1) {
			itoast('请添加支持的图片附件后缀类型', '', '');
		}
	...
	$upload['images']['width'] = intval($upload['images']['width']);
	if (empty($upload['images']['width'])) {
		$upload['images']['width'] = 800;
	}
	if (!empty($upload['images']['extentions']) && is_array($upload['images']['extentions'])) {
		$upload['images']['extentions'] = implode("\n", $upload['images']['extentions']);
	}
...
if($do == 'ftp') {
	...
	if (true === $ftp->connect()) {
			...
			if (!empty($image) && strexists($image['mime'], 'images')) {
				iajax(0,'配置成功');
			} else {
				iajax(-1, '配置失败，FTP远程访问url错误');
			}
