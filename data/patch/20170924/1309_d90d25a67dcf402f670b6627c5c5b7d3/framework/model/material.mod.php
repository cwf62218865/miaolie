...
function material_strip_wechat_image_proxy($content) {
	...
	if (!empty($match_wechat[1])) {
		foreach ($match_wechat[1] as $val) {
			$wechat_thumb_url = urldecode(str_replace($_W['siteroot'] . 'web/index.php?c=utility&a=wxcode&do=images&attach=', '', $val));
...
function material_upload_limit() {
	...
	if (isset($upload['images']['limit']) && (bytecount($upload['images']['limit'].'kb')>0)){
		$upload_limit['images'] = bytecount($upload['images']['limit'].'kb');
	}
	if (isset($upload['images']['limit']) && (bytecount($upload['audio']['limit'].'kb')>0)){
	...
	$conditions['uniacid'] = $_W['uniacid'];
		...
		switch ($type) {
			...
			case 'video' :
				...
				break;
			default :
				$conditions['type'] = $server == MATERIAL_LOCAL ? ATTACH_TYPE_IMAGE : 'images';
				break;
		}
		if ($server == 'local') {
...
function material_to_local($resourceid, $uniacid, $uid, $type = 'images') {
...
function material_network_image_to_local($url, $uniacid, $uid) {
	return material_network_to_local($url, $uniacid, $uid, 'images');
}



function material_network_to_local($url, $uniacid, $uid, $type = 'images') {
	...
	$data = array('uniacid' => $uniacid, 'uid' => $uid,
		'filename' => $filename,
		'attachment' => $path,
		'type' => $type == 'images' ? ATTACH_TYPE_IMAGE : ($type == 'audio'||$type == 'voice' ? ATTACH_TYPE_VOICE : ATTACH_TYPE_VEDIO),
...
function material_to_wechat($attach_id, $uniacid, $uid, $acid, $type = 'images') {
...
function material_network_image_to_wechat($url, $uniacid, $uid, $acid) {
	return material_network_to_wechat($url, $uniacid, $uid, $acid, 'images');
}


function material_network_to_wechat($url, $uniacid, $uid, $acid, $type = 'images') {
