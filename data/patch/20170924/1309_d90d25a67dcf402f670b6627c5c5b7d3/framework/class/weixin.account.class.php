...
class WeiXinAccount extends WeAccount {
	...
	public function queryAvailableMessages() {
		$messages = array('text', 'images', 'voice', 'video', 'location', 'link', 'subscribe', 'unsubscribe');
	...
	public function fansSendAll($group, $msgtype, $media_id) {
		$types = array('text' => 'text', 'images' => 'images', 'news' => 'mpnews', 'voice' => 'voice', 'video' => 'mpvideo', 'wxcard' => 'wxcard');
	...
	public function uploadMedia($path, $type = 'images') {
		if(empty($path)) {
			return error(-1, '参数错误');
		}
	...
	public function downloadMedia($media_id, $savefile = true) {
		$mediatypes = array('images', 'voice', 'thumb');
